...
test('lists sheets in document order', () => {
  const workbook = readWorkbookPart(packageOf(TWO_SHEETS))
...
test('defaults to the 1900 date system', () => {
  assert.equal(readWorkbookPart(packageOf(TWO_SHEETS)).date1904, false)
})
...
test('reads the 1904 date system flag', () => {
  ...
  assert.equal(readWorkbookPart(packageOf(xml)).date1904, true)
})
...
test('rejects a package with no root relationships', () => {
  ...
  assert.throws(() => readWorkbookPart(bytes), /_rels\/\.rels/)
})
...
test('skips a sheet whose relationship does not resolve', () => {
  ...
  const workbook = readWorkbookPart(packageOf(xml))
...
test('reads a very hidden sheet', () => {
  ...
  assert.equal(readWorkbookPart(packageOf(xml)).sheets[0]?.state, 'veryHidden')
})
...
test('accepts a relationship prefix other than r', () => {
  ...
  assert.equal(readWorkbookPart(packageOf(xml)).sheets[0]?.path, 'xl/worksheets/sheet1.xml')
})

test('skips a sheet with no relationship attribute at all', () => {
  const xml = `<workbook><sheets><sheet name="A" sheetId="1"/></sheets></workbook>`

  assert.deepEqual(readWorkbookPart(packageOf(xml)).sheets, [])
})
...
test('skips a sheet whose relationship is external', () => {
  ...
  const workbook = readWorkbookPart(packageOf(xml, { 'xl/_rels/workbook.xml.rels': rels }))
...
test('defaults name and sheetId when the attributes are absent', () => {
  ...
  const sheet = readWorkbookPart(packageOf(xml)).sheets[0]
...
test('reads a workbook that has no relationships part', () => {
  ...
  assert.deepEqual(readWorkbookPart(writeContainer({ parts })).sheets, [])
})
